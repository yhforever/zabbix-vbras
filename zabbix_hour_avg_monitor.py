...
def run_select(start_date,end_date,row):
    ...
    start_time=(start_date - datetime.timedelta(hours=1)).strftime("%Y-%m-%d %H")
    ...
    #    print "t value %s"%t
        ...
        get_netcard_in_peak_sql="select from_unixtime(hi.clock,'%%Y-%%m-%%d %%T') as Date,g.name as vBras_Name,hi.value_max as Netcard_In_Peak from hosts_groups hg join groups g on g.groupid = hg.groupid join items i on hg.hostid = i.hostid join hosts h on h.hostid=i.hostid join trends_uint hi on  i.itemid = hi.itemid  where  i.key_='net.if.in[eth1]' and hi.clock >= UNIX_TIMESTAMP('%s:00:00') and  hi.clock <= UNIX_TIMESTAMP('%s:00:00') and g.name like '%%vbras server%%';"%(start_time,t)
        ...
        get_netcard_out_peak_sql="select from_unixtime(hi.clock,'%%Y-%%m-%%d %%T') as Date,g.name as vBras_Name,hi.value_max as Netcard_Out_Peak from hosts_groups hg join groups g on g.groupid = hg.groupid join items i on hg.hostid = i.hostid join hosts h on h.hostid=i.hostid join trends_uint hi on  i.itemid = hi.itemid  where  i.key_='net.if.out[eth1]' and  hi.clock >= UNIX_TIMESTAMP('%s:00:00') and  hi.clock <= UNIX_TIMESTAMP('%s:00:00')  and g.name like '%%vbras server%%';"%(start_time,t)
...
if __name__ == "__main__":
    ...
    conn=MySQLdb.connect(host='172.16.128.245',user='root',passwd='123456',port=3306,charset="utf8")
    cur=conn.cursor()
    conn.select_db('zabbix')
    sheetname=['业务巡检项目'.encode('utf-8')]
    #sheetname="sheet"
    wb = xlwt.Workbook()
    ...
    #title=['Time'.encode('utf-8'),'Group'.encode('utf8'),'Bgp route'.encode('utf8'),'Bgp route peak'.encode('utf8'),'Traffic in'.encode('utf8'),'Traffic In Peak'.encode('utf8'),'Traffic Out'.encode('utf8'),'Traffic Out Peak'.encode('utf8'),'Nat user'.encode('utf8'),'Nat user peak'.encode('utf8'),'Nat session'.encode('utf8'),'Nat session peak'.encode('utf8')]
    title = ['时间'.encode('utf-8'), '网元'.encode('utf8'), 'Bgp路由数目'.encode('utf8'), 'Bgp路由峰值'.encode('utf8'), 'In流量(bps)'.encode('utf8'), 'In流量峰值(bps)'.encode('utf8'),'Out流量(bps)'.encode('utf8'), 'Out流量峰值(bps)'.encode('utf8'), 'Nat用户数'.encode('utf8'), 'Nat用户数峰值'.encode('utf8'), 'Nat Session'.encode('utf8'), 'Nat session峰值'.encode('utf8')]
    for j in range(0,8):
    ...
    wb.save('/tmp/zabbix_log/vbras_monitor_hourly_report.xls')
    cur.close()
    conn.close()
    #follow is send mail
    server = {'name':'smtp.163.com', 'user':'yanghao19890203', 'passwd':'yxl19861207', 'port':25}
    fro = 'yanghao19890203@163.com'
    to = ['yangh@certusnet.com.cn']
    subject = '梅州移动政务网专线业务vBRAS巡检报表[%s:00 to %s:00]'%(start_date.strftime("%Y-%m-%d %H"),end_date.strftime("%Y-%m-%d %H"))
    #print "subject"
    #print subject
    text = 'Dear,all:\n附件为梅州移动政务网专线业务vBRAS巡检报表,请查收.'
    files = ['/tmp/zabbix_log/vbras_monitor_hourly_report.xls']
